...
<body>
	...
	<br>
	<button id="btn12">ex44/sub06 post 요청 form -> obj -> json</button><br>
...
document.querySelector("#btn12").addEventListener("click", function() {
	...
	const location1 = document.querySelector("#locationInput2").value;
	const since = document.querySelector("#sinceInput2").value;
	const obj = {
		name : name,
		job : {location: location1, since : since}
	};
	const data = JSON.stringify(obj);
...
document.querySelector("#btn8").addEventListener("click", function() {
	...
	const data = JSON.stringify(o); //문자열로 변환
...
document.querySelector("#btn5").addEventListener("click", function() {
	fetch(ctx + "/ex44/sub05", {
		...
		body : `{"address":"seoul", "hasCar": true, "drink":["콜라", "커피"], "age":27}`
	})
});
...
document.querySelector("#btn3").addEventListener("click", function() {
	fetch(ctx + "/ex44/sub03", {
		method: "post",
		headers : {
			"Content-Type" : "application/json" //객체로 사용가능
		},
...
document.querySelector("#btn2").addEventListener("click", function() {
	fetch(ctx + "/ex44/sub02", {
		method: "post",
		body : '{"name":"son", "address":"seoul"}' //스트링으로만 가능 객체로는 불가능
	});
});
